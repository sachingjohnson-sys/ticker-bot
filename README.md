# Ticker Bot
A Node.js application that connects to the Uphold Ticker API
to monitor cryptocurrency and currency pairs, generating alerts when thresholds are breached.
...
Supports multiple concurrent bots using unique botIds.
...
Code Structure	      Layered modules (controller → service → DAO → DB) to maintain single responsibility.
...
Create a .env file in `both` the root folder and the alerts_api/ directory based on .env.example.
...
In production, these values should be injected securely from a secrets manager or CI/CD environment.
...
Endpoint:
POST http://localhost:3000/alerts/filter
